...
def generate_image(client, prompt, uploads, model, aspect_ratio, resolution):
    contents = [prompt]
    ...
    if generated is None:
        raise RuntimeError(
            ...
            "Tente reduzir o número de referências ou alterar o prompt."
        )

    return generated, "\n".join(explanation)


def generate_video(client, prompt, main_image_upload, reference_uploads, resolution, aspect_ratio):
    # A API do Veo aceita uma imagem principal e até 3 imagens de referência.
    first_image = None
    ...
    progress = st.progress(0, text="Enviando o trabalho para o Veo…")
    checks = 0

    while not operation.done:
        checks += 1
        progress.progress(min(95, 10 + checks * 5), text="Gerando vídeo… aguarde.")
        time.sleep(10)
        operation = client.operations.get(operation)

    progress.progress(100, text="Vídeo concluído.")

    if not operation.response or not operation.response.generated_videos:
        raise RuntimeError("O Veo terminou sem retornar um vídeo.")

    generated_video = operation.response.generated_videos[0].video

    # Alguns ambientes devolvem bytes diretamente; em outros, o objeto de arquivo
    ...
    out = io.BytesIO()
    try:
        client.files.download(file=generated_video, destination=out)
        return out.getvalue()
    except Exception:
        temp = Path("video_result.mp4")
        client.files.download(file=generated_video, destination=str(temp))
...
A primeira imagem é a fotografia principal. As demais são referências documentais.
...
Movimentos humanos mínimos e plausíveis; nada de deformar rostos, mãos, roupas, casas ou veículos.
Não modernizar o cenário.
Não inserir pessoas, prédios, carros, placas ou objetos que não estejam nas referências.
Fotografia histórica restaurada ganhando vida, com movimento realista e cinematográfico sutil."""


# ---------------- UI ----------------

st.title("📷 Reconstrução Histórica com IA")
st.caption("Fotografias antigas → reconstrução/restauração → vídeo histórico com movimento")
...
tab_img, tab_video, tab_config = st.tabs(
    ["🖼️ Reconstruir fotografia", "🎬 Gerar vídeo", "⚙️ Configuração da API"]
)

with tab_img:
    st.subheader("1. Envie as referências")
    st.write(
        "Use a primeira imagem como fotografia principal. As outras podem ser "
        "fotos da mesma casa, pessoa, paisagem, veículo ou um desenho/esboço."
    )
    ...
    st.subheader("2. Defina como a reconstrução deve ser feita")
    prompt = st.text_area(
        "Prompt",
    ...
    with c1:
        model_label = st.selectbox("Modelo", list(IMAGE_MODELS.keys()))
    with c2:
        aspect_ratio = st.selectbox(
            "Proporção",
            ["16:9", "4:3", "3:2", "1:1", "3:4", "9:16"],
            index=0,
        )
    with c3:
        resolution = st.selectbox("Resolução", ["1K", "2K", "4K"], index=1)

    if main_photo:
        st.image(main_photo, caption="Fotografia principal", use_container_width=True)
    ...
    if st.button(
        ...
        client = make_client()
        refs = [main_photo, person_ref, house_ref, landscape_ref, sketch_ref, extra_ref]
        refs = [x for x in refs if x is not None]

        with st.spinner("A IA está reconstruindo a fotografia…"):
            try:
                ...
                st.success("Reconstrução concluída.")
                st.image(image, caption="Resultado", use_container_width=True)
                if explanation:
                    with st.expander("Observação retornada pelo modelo"):
                        st.write(explanation)
            except Exception as exc:
                st.error(f"Erro ao gerar a fotografia: {exc}")
...
with tab_video:
    st.subheader("Transforme a fotografia em um pequeno filme histórico")
    st.info(
        "O Veo 3.1 gera vídeos curtos. Para uma reconstrução mais consistente, "
        "use a imagem reconstruída como imagem principal e até 3 referências."
    )

    generated_img = st.session_state.get("last_image")
    if generated_img is not None:
        st.image(generated_img, caption="Fotografia reconstruída disponível", width=500)

    video_start = st.file_uploader(
        "Imagem inicial do vídeo (se deixar vazio, use a reconstrução acima)",
        type=["jpg", "jpeg", "png", "webp"],
        key="video_start",
    )
    ...
    start_available = bool(video_start or generated_img)
    if st.button(
        "🎬 GERAR VÍDEO COM VEO 3.1",
        type="primary",
        use_container_width=True,
        disabled=not bool(api_key and start_available),
    ):
        client = make_client()

        # Se o usuário não enviou uma imagem inicial, salva temporariamente
        # a reconstrução em memória para alimentar o Veo.
        if video_start is None:
            temp = io.BytesIO()
            generated_img.save(temp, format="PNG")
            class MemoryUpload:
                type = "image/png"
                def getvalue(self):
        ...
        refs = [video_ref1, video_ref2, video_ref3]
        refs = [x for x in refs if x is not None]

        with st.spinner(
            "Gerando o vídeo no Veo 3.1. Isso pode levar alguns minutos…"
        ):
            try:
                ...
                st.error(f"Erro ao gerar o vídeo: {exc}")

with tab_config:
    st.subheader("🔑 Como configurar a API do Google Gemini")
...
**Não publique esse arquivo no GitHub.** O projeto já inclui um `.gitignore`
que bloqueia `secrets.toml`.
...
Depois de configurar a chave, volte para **Reconstruir fotografia**, envie uma
foto antiga e clique em **RECONSTRUIR FOTOGRAFIA**.

### Modelos incluídos

- **Nano Banana 2** (`gemini-3.1-flash-image`) — padrão para reconstrução.
- **Nano Banana Pro** (`gemini-3-pro-image`) — opção para trabalhos mais
  complexos.
- **Nano Banana** (`gemini-2.5-flash-image`) — modelo legado.
- **Veo 3.1** (`veo-3.1-generate-preview`) — geração de vídeo.
"""
    )

    if api_key:
        st.success("✅ API key encontrada. A aplicação está pronta para uso.")
    else:
        st.error("❌ API key não encontrada.")

st.divider()
st.caption(
    "Aplicação preparada para restauração/reconstrução histórica com referências "
    "fotográficas. Verifique os direitos de uso das imagens enviadas."
)
